...
export const checkSubscription = async (req, res, next) => {
  try {
    ...
    // Auto-expire trial
    if (status === 'TRIAL' && now > dealer.trialEndsAt) {
    ...
    if (status === 'ACTIVE' && dealer.subscriptionEndsAt && now > dealer.subscriptionEndsAt) {
      await prisma.dealer.update({
        where: { id: dealer.id },
        data: { subscriptionStatus: 'EXPIRED' }
      });
    ...
    const daysLeft = status === 'TRIAL'
      ? Math.ceil((dealer.trialEndsAt - now) / (1000 * 60 * 60 * 24))
      : status === 'ACTIVE' && dealer.subscriptionEndsAt
      ? Math.ceil((dealer.subscriptionEndsAt - now) / (1000 * 60 * 60 * 24))
      : 0;

    res.json({ status, daysLeft, trialEndsAt: dealer.trialEndsAt, subscriptionEndsAt: dealer.subscriptionEndsAt });
  } catch (err) { next(err); }
};
