...
export default function CarFormModal({ car, onClose, onSuccess }) {
      ...
      <div className="w-full max-w-2xl bg-dark-800 border border-white/10 rounded-2xl shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Make" required><Input value={form.make} onChange={set('make')} placeholder="e.g. Toyota" required /></FormField>
            <FormField label="Model" required><Input value={form.model} onChange={set('model')} placeholder="e.g. Land Cruiser" required /></FormField>
            <FormField label="Year" required><Input value={form.year} onChange={set('year')} placeholder="2022" type="number" required /></FormField>
            <FormField label="Price (KES)" required><Input value={form.price} onChange={set('price')} placeholder="4500000" type="number" required /></FormField>
            <FormField label="Mileage (km)"><Input value={form.mileage} onChange={set('mileage')} placeholder="35000" type="number" /></FormField>
            <FormField label="Color"><Input value={form.color} onChange={set('color')} placeholder="e.g. Pearl White" /></FormField>
            <FormField label="Engine"><Input value={form.engine} onChange={set('engine')} placeholder="e.g. 3.0L V6 Turbo" /></FormField>
            <FormField label="Horsepower"><Input value={form.horsepower} onChange={set('horsepower')} placeholder="272" type="number" /></FormField>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <FormField label="Condition"><Select value={form.condition} onChange={set('condition')} options={CONDITIONS} /></FormField>
            <FormField label="Body Type"><Select value={form.bodyType} onChange={set('bodyType')} options={BODY_TYPES} /></FormField>
            <FormField label="Fuel Type"><Select value={form.fuel} onChange={set('fuel')} options={FUELS} /></FormField>
            <FormField label="Transmission"><Select value={form.transmission} onChange={set('transmission')} options={TRANSMISSIONS} /></FormField>
          </div>

          <FormField label="Description">
            <textarea value={form.description} onChange={set('description')} rows={3}
              placeholder="Describe the car's features, history, condition..."
